...
export function executeAbilityCommand(
  ...
  switch (command.type) {
    case "ability.use":
      return executeUseAbility(state, command);
    case "noble_phantasm.prepare":
      return executePrepareNoblePhantasm(state, command);
    case "noble_phantasm.release":
      return executeReleaseNoblePhantasm(state, command);
  }
}
...
function executeUseAbility(state: GameState, command: UseAbilityCommand): AbilityCommandResult {
  ...
  if (definition.target === "self") {
    ...
    if (!target) return failure("target_not_found", `Target ${command.targetId} does not exist`);
    if (!legalTargets.some(candidate => candidate.id === target.id)) {
      return failure("ability_out_of_range", `${target.name} is not a legal target for ${definition.name}`);
    }
    targets = [target];
  }

  if (command.abilityId === "lancer_battle_continuation" && actor.battleContinuationActive) {
    return failure("ability_no_effect", "Battle Continuation is already prepared");
  }
  if (command.abilityId === "archer_guard_support" && actor.reactionAvailable && actor.guardBonus >= 1) {
  ...
  const events: DomainEvent[] = [
    ...
    {
      type: "battle.mana_spent",
      sequence: nextSequence(),
      battleId: battle.id,
      unitId: actor.id,
      amount: definition.manaCost,
      reason: "ability",
    },
  ];
  ...
  for (const effect of definition.effects) {
    switch (effect.type) {
      case "damage": {
        ...
        events.push(
          ...
          },
          {
            ...
            sourceId: actor.id,
            targetId: target.id,
            amount: calculateAbilityDamage(actor, target, effect.power),
            kind: "ability",
          },
        );
        break;
      ...
      case "guard_support":
        events.push({
          type: "ability.guard_support_activated",
          sequence: nextSequence(),
          battleId: battle.id,
          servantId: actor.id,
          guardBonus: effect.guardBonus,
        });
        break;
      case "dash_strike": {
        const target = targets[0];
      ...
      case "battle_continuation":
        events.push({
          type: "ability.battle_continuation_activated",
          sequence: nextSequence(),
          battleId: battle.id,
          servantId: actor.id,
        });
...
function executePrepareNoblePhantasm(
  state: GameState,
  command: PrepareNoblePhantasmCommand,
): AbilityCommandResult {
  const battle = state.battle;
  if (battle.id !== command.battleId) return failure("battle_not_found", `Battle ${command.battleId} does not exist`);
  const servant = battle.units[command.servantId];
  const target = battle.units[command.targetId];
  if (!servant) return failure("unit_not_found", `Servant ${command.servantId} does not exist`);
  if (!target) return failure("target_not_found", `Target ${command.targetId} does not exist`);
  ...
  if (!findLegalNoblePhantasmTargets(battle, servant.id).some(candidate => candidate.id === target.id)) {
    return failure("noble_phantasm_target_invalid", `${target.name} cannot be locked by ${definition.name}`);
  }
  ...
  return {
    ...
    events: [
      ...
      },
      {
        ...
        unitId: servant.id,
        amount: definition.manaCost,
        reason: "noble_phantasm",
      },
      {
        type: "noble_phantasm.preparation_started",
        ...
        interruptThreshold: definition.interruptThreshold,
      },
    ],
  };
}

function executeReleaseNoblePhantasm(
  state: GameState,
  command: ReleaseNoblePhantasmCommand,
): AbilityCommandResult {
  const battle = state.battle;
  if (battle.id !== command.battleId) return failure("battle_not_found", `Battle ${command.battleId} does not exist`);
  const servant = battle.units[command.servantId];
  ...
  if (!servant.mainActionAvailable) return failure("main_action_unavailable", `${servant.name} has spent the main action`);
  const nobleState = servant.noblePhantasm;
  if (!nobleState || nobleState.phase !== "ready" || !nobleState.targetId) {
    return failure("noble_phantasm_not_ready", `${servant.name}'s noble phantasm is not ready`);
  }
  const target = battle.units[nobleState.targetId];
  if (!target || target.defeated || target.factionId === servant.factionId) {
    return failure("noble_phantasm_target_invalid", "The locked noble-phantasm target is no longer valid");
  }
  const definition = getNoblePhantasmDefinition(nobleState.definitionId);
  if (!definition) return failure("noble_phantasm_unavailable", `Unknown noble phantasm ${nobleState.definitionId}`);
  if (hexDistance(servant.position, target.position) > definition.range) {
    return failure("noble_phantasm_out_of_range", `${target.name} escaped the noble-phantasm range`);
  }
