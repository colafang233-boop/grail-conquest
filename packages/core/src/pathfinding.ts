...
export function findReachableHexes(
  battle: BattleState,
  unitId: UnitId,
): Readonly<Record<string, ReachableHex>> {
  ...
  while (frontier.length > 0) {
    ...
    for (const neighbor of getHexNeighbors(parseHexKey(current.key))) {
      ...
      if (!tile || tile.blocked || occupied.has(neighborKey)) {
        continue;
      }
  ...
  for (const [destinationKey, cost] of costs) {
    ...
    while (cursor !== startKey) {
      ...
      if (!parent) {
        throw new Error(`Path reconstruction failed for ${destinationKey}`);
      }

      cursor = parent;
    }

    reachable[destinationKey] = {
      coord: parseHexKey(destinationKey),
      cost,
